...
def edges_eval_img(im, gt, out="", thrs=99, max_dist=0.0075, thin=True, need_v=False, workers=1):
    ...
    if workers == 1:
        for k_ in range(k):
            ...
            for g in gt: # 如果真值只有一个 这里也就只算一次了
            ...
            cnt_sum_r_p[k_, :] = [np.sum(match_g), np.sum(all_g), np.count_nonzero(match_e), np.count_nonzero(e1)] # 打印的那四位
        ...
        def _process_thrs_loop(_edge, _gt, _eps, _thrs, _thin, _max_dist, _indices, _queue): #多线程的计算过程
            for _k in _indices:
    ...
    info = np.concatenate([thrs[:, None], cnt_sum_r_p], axis=1) #txt那四个东西
    if out:
...
def edges_eval_dir(res_dir, gt_dir, cleanup=0, thrs=99, max_dist=0.0075, thin=True, workers=1):
    ...
    ids = [os.path.split(file)[-1] for file in glob.glob(os.path.join(gt_dir, "*.png"))] #读gt文件夹的样本用的，剥离成只有id的list，哇 这玩意儿真的是脱裤子放屁
    # print(ids)
    for ci, i in enumerate(ids):
        ...
        if os.path.isfile(res):
            continue
        im = os.path.join(res_dir, "{}.png".format(i)) #此代码对于 nms后的输入一定是png格式的，gt一定必须是mat的
        # gt = os.path.join(gt_dir, "{}.mat".format(i))
        gt = os.path.join(gt_dir, "{}.png".format(i))
